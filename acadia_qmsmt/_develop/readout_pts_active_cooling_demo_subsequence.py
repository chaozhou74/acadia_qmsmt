...
@dataclass
class ActiveCoolingRuntime(SingleQubitRuntime, Runtime):
    """
    ...
    def main(self):
        ...
        q_rotation_pi = self.allocate_waveform_mem("q_stimulus", "q_rotation")
        q_rotation_pio2 = self.allocate_waveform_mem("q_stimulus", "q_rotation")
        ro_drive = self.allocate_waveform_mem("ro_stimulus", "ro_drive")
        ro_pts_0 = self.allocate_waveform_mem("ro_capture", "ro_pts")
        ro_pts_1 = self.allocate_waveform_mem("ro_capture", "ro_pts")
        ro_pts_2 = self.allocate_waveform_mem("ro_capture", "ro_pts")
        ...
        def sequence(a: Acadia):
            ...
            # wait for the first msmt to finish
            with a.sequencer().repeat_until(a.cmacc_done(capture_stream)):
                pass


            ## Step 2: measure and flip qubit until qubit is in g
            self.subsqeuence_cool_qubit(a, self.ro_capture, ro_pts_1, ro_drive_wf=ro_drive, capture_blank_wf=blank_wf,
                                        q_pi_pulse_wf=q_rotation_pi, q_blank_wf=q_blank_wf)

 

            ## Step 3: Do a final msmt
            capture_stream_2, kernel = a.configure_cmacc(self.channel_objs["ro_capture"], kernel=kernel,
                                                            reset_fifo=False, accumulator_done=False)
            a.cmacc_load(capture_stream_2, cmacc_offset)

            with a.channel_synchronizer():
                if capture_delay != 0:
                    a.schedule_waveform(blank_wf)
                a.schedule_waveform(ro_drive)
                a.stream(capture_stream_2, ro_pts_2)
...
if __name__ == "__main__":
    ...
    rt.deploy("10.66.3.198", "readout_pts_active_cooling_demo_subsequence", files=rt.FILES)
    rt.display()
