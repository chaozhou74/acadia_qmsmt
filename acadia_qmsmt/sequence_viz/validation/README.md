...
* the card wires **DAC pairs to ADC pairs in reverse order** — ADC0 hears DAC14. This harness's
  own `stimulus0..3` therefore sit on **DAC14/12/10/8** so that `capture0..3` (ADC0/2/4/6, the
  four with dedicated stream inputs) each hear their own stimulus. They used to be DAC0/2/4/6,
  paired by index, and every board run recorded ~2 counts of crosstalk instead of the pulse —
  bug 41. `make_xm650_config.py` writes and asserts the pairing; do not hand-edit it here;
...
| 40 | Three cavities cooling on one interleaved ladder were drawn with the SAME swap length in every round, and 9 rounds where the board runs 3 | `cool_modes` puts the pairing loop INSIDE the round loop so the modes descend together, so one pass advances its single cache pointer once per PAIRED MODE. Two consequences, both wrong in the model: a pointer loop's pass count was taken as its WORD RANGE (rounds x modes), and `_pointer_length` counted reads per `(channel, register)` -- so each cavity got words `start+0, start+1, start+2` instead of its own stride-K slice of the round-major cache. The cavities' level-1 swaps are 176.4 / 1072.9 / 343.9 ns, so equal widths are physically impossible, yet nothing errored: the picture was simply a schedule the cache does not contain. How far one pass moves the pointer IS in the program -- `pulse_cep()` emits an instruction whose only trace is the STP's `dsp_cep` field, invisible to every `d1`/`d2` predicate -- so `decode_program` now carries it (`Instr.cep`), `describe_cep_sites` lists the addresses per DSP, and `pointer_cep_per_pass` counts the ones inside the innermost enclosing back-edge. Reads are counted per POINTER, in execution order, which reproduces the walking pointer whichever channel a read lands on. Case `interleaved_stretch`, gated by `counting_round_timing.py --phase q7` |
| 41 | Every board run through this harness had been recording CROSSTALK, not its own pulses | The config paired `stimulus_k` with `capture_k` by INDEX (DAC0/2/4/6 with ADC0/2/4/6). The XM650 card wires DAC pairs to ADC pairs in REVERSE order -- `DAC n -> ADC 14 - 2*(n//2) + n%2`, which this README's own section 'The board' already stated -- so every capture listened to a line nothing drove. `single` recorded a peak of 3.0 counts where the direct path gives 133, i.e. the card's ~2 counts of crosstalk, and `find_pulses` (which scales to each channel's own noise) duly reported 18-38 'pulses' of 5-20 ns. `--dryrun` stayed green throughout. The stimuli are now chosen FROM the captures -- ADC0/2/4/6 are the four with dedicated stream inputs, and the DACs they hear are 14/12/10/8 -- `make_xm650_config.retune_generic` writes the channels rather than leaving them alone (which ADC hears which DAC is a property of the CARD, not something a generic config can be neutral about) and asserts the pairing against `DAC_TO_ADC` at import. `CHANNEL_OF` is read from the config, and per-trace via `channel_of(trace)` so an archived run recorded before the move still scores. After: `single` peaks 5.7e3-1.4e4 counts |
| 42 | Every case deployed through `prep_selector_timing.deploy` was recorded in a 4 us window regardless of its length | `deploy` traced ONE probe runtime twice -- `unsafe_reason(probe)` then `capture_window_for(probe)` -- and a runtime can only be traced once (its Acadia keeps the compiled program), so the window call raised and fell back to the `CAPTURE_WINDOW_S` constant. Each now gets its own probe. `capture_window_for` also traced with `capture_points=False`, which leaves every register-driven length at `resolve_indeterminate` (0) -- the same trap the neighbouring `unsafe_reason` docstring documents -- so a 7.66 us `interleaved_stretch` measured 4.29 us and its window came out 5.5 us: the last stretched pulse was cut from 650 ns to 340, which reads as a runtime that stopped playing |
| 43 | A `use_stretch` pulse's PREDICTED width was its first half only | `predict`/`regions_of` built each region from commands carrying a pulse NAME, and a stretched pulse compiles to ARB + CONST_CONT + ARB_CONT with only the first named. `interleaved_stretch` predicted 50 ns regions against 300/500/695 ns measured -- the model had resolved the lengths correctly from the cache and the REPORT threw them away. Continuations are now merged into the region they continue, and the widths agree to 0-10 ns (the known hann half-height systematic). Width is the entire claim for a register-driven length, so it had to be the whole pulse |
